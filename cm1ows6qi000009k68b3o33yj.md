...
With the context of the type of information, it’s still not enough to be a problem. When this information is shared with other colleges or even publicly, it becomes punishable.
...
However, one can provide the OTP to authorize with their institute email and their username and password to someone outside of KGP (not a KGPian). Yes, true. The same can happen with ERP credentials as well. It doesn’t make sense to see this as a problem in ‘Naarad+MFTP’, but it’s fine when it happens with ERP. Still, we log all the connections made to Naarad, in case they are needed, with uniquely identifiable information such as *username* and *IP address*.
...
<mark>With the history of KOSS and metaKGP intermingled, the history of metaKGP was passed on, directly or indirectly, in KOSS</mark>. The reasons for intermingled history can be summarised as follows:
...
If you care about the monetary aspect, we are actually running into loss :D
...
* Having no freakin’ idea of what are the previous grade statistics of a subject whose name/syllabus sounds “cool” and you plan to take it as an elective or as an additional to end up in *absolute grading* ([Kronos](https://kronos.streamlit.app))
...
<mark>The boundaries between being unaware and stupid are getting blurred here</mark>. Surely, I understand where they might be coming from - they believe metaKGP ought to serve them for all the various reasons I have discussed earlier, or metaKGP is supported by the administration, so we get some ‘<mark>exclusive</mark>’ data from them. We build the data ourselves, which is scattered all over the ERP or other resources. No one pops out of heaven to provide us with this data magically.
...
* Before anything, ensure you have read the corresponding documentation - if there is any. Efforts were put into writing that documentation as thoroughly as possible. In fact, I spent 3 hours writing the [documentation for Naarad](https://bit.ly/naarad-guide) to get stupid queries again (from those who have already read the documentation). Here is one of those instances.
